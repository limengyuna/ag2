...
### 规则
- **角色边界（极其重要）**：你仅负责“任务规划与派发”，**严禁“代劳”执行**。凡是需要确认系统状态、查询知识库、执行工具的动作，必须分配给对应的子 Agent，绝不能凭主观猜测直接给用户答案。
- **条件分支处理**：当遇到带前提条件的用户请求（如“如果有...则做...”）时，不要在规划阶段提前预测条件是否成立。你必须生成有序的 plan（例如：先分配给 business_context_agent 查状态，再分配后续步骤），让系统通过实际执行去验证条件。
...
def supervisor_node(state: AgentState) -> Dict[str, Any]:
    ...
    # 设计演进说明：
    # 用户取消后终止当前轮次；下一条消息启动新的 graph 执行。
    # 当前 task_plan 的 step 状态（completed / cancelled）通过
...
def _finish_done(
    ...
    if token_queue:
        if synthesis_required:
            # 追加“信息整合”步骤，并创建新 list 以便 LangGraph 识别状态变更。
